...
func TestServiceAccountMappedLoggedWithoutResources(t *testing.T) {
	// A named account is enough for a template (ADR-0012), so the INFO fires
...
// Membership decides which BuildConfigs get W72 instead of W9, so a name added
...
func TestServiceAccountBuilderAndDeployerWarnedSeparately(t *testing.T) {
	...
	// so the migration-carries-it warning would be wrong for them. They get W72
	...
	// itself, so adding a fourth name would change which BuildConfigs get W72
...
func TestDroppedServiceAccountWithPullSecretWarnsTwiceWithoutContradiction(t *testing.T) {
	// builder plus a pull secret is the one input where W8 and W72 both fire on
	// one Build. W8 must not claim crane migrates the account, because W72 says
