...
func TestConvertSourceEnvBuildEnv(t *testing.T) {
	tests := []struct {
		...
		{
			// BUILD-2500 coderabbit/aufi review: an optional key missing at
			// BuildRun time leaves the variable unset, so build-env's
			// NAME=$(NAME) resolves to the literal text instead of an
			// omitted variable. The entry stays; only a warning names it.
			name: "optional secret and configmap keys",
			env: `[
				{"name": "OK", "value": "1"},
				{"name": "TOKEN", "valueFrom": {"secretKeyRef": {"name": "creds", "key": "token", "optional": true}}},
				{"name": "FLAG", "valueFrom": {"configMapKeyRef": {"name": "cfg", "key": "flag", "optional": true}}}
			]`,
			wantBuildEnv: []string{"OK=$(OK)", "TOKEN=$(TOKEN)", "FLAG=$(FLAG)"},
			wantWarn:     []string{"sets sourceStrategy.env TOKEN, FLAG from an optional Secret or ConfigMap key"},
			wantOutcome:  OutcomeConvertedWithWarnings,
			wantEnv:      3,
		},
	}
