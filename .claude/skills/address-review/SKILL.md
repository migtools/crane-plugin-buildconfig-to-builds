...
`$SCRATCH/edit-pr-approved.json` with the Write tool and add `--approved` followed by that
file's resolved path: the user approved these fixes in the table, and `/edit-pr`'s tests
still gate the push.
