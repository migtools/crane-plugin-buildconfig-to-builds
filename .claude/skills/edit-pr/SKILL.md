...
allowed-tools: [Bash, Read, Write, Edit, AskUserQuestion, Skill, EnterWorktree]
user_invocable: true
---
...
1. For each target, first `git -C "$WORK" add -- <files>`, then commit exactly those files
   with `git commit --only -s -S -m tmp -- <files>`, so every piece has a SHA.
2. Check `git -C "$WORK" status --porcelain --untracked-files=no`. Anything it lists is a
   tracked change outside the files just committed. The reset in the next step would
   destroy it, and Step 2 promised to leave it alone. Stop and name those files instead of
   resetting. The user commits or clears them before this path can run.
3. Note every SHA in the order the new layout needs, then
   `git -C "$WORK" reset --hard <merge-base with origin/main>`.
4. For each commit in the new layout: `git -C "$WORK" cherry-pick -n <sha> <sha> ...` for
...
The first check only applies after a rebuild. When `git config user.signingkey` is set,
every commit here must show `G`. `U` counts too, a good signature on a key that just isn't
marked trusted. With no signing key configured, `N` is expected and is not a stop. A commit
this run left untouched, already showing `N`, is named in the report, not treated as a
failure. After `--rebase`, the second check compares against the rebased tip (before the
new work came back) instead of the backup tag, which sits on the old base.
...
Compare the fork's tip now against `headRefOid`, the one Step 1 read from the PR:

```bash
git -C "$WORK" fetch fork "$BRANCH"
git -C "$WORK" rev-parse "fork/$BRANCH"                                # current fork tip
```

Stop if that does not equal `headRefOid`. Something pushed to the fork branch after Step 1
read it, and pushing now would overwrite that work unseen. When it matches, tag and push
with a lease anchored to the `headRefOid` value Step 1 read, not to whatever was just
fetched:

```bash
git -C "$WORK" tag "archive/$FORK_OWNER-old-$BRANCH" "fork/$BRANCH"
git -C "$WORK" push fork "archive/$FORK_OWNER-old-$BRANCH"
git -C "$WORK" push --force-with-lease="$BRANCH:<headRefOid>" fork "$BRANCH:$BRANCH"
```

For the maintainer's fork the tag is `archive/psrvere-old-<branch>`. If that tag already
exists on another commit, add the short SHA of the current tip:
`archive/$FORK_OWNER-old-$BRANCH-<short tip>`. When only new commits were added on top, a
...
between this fetch and the push: stop, say so, and push nothing else.
