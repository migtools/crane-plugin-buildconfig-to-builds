...
CHANGED=$( { git -C "$WORK" diff --no-ext-diff --name-only "main...HEAD";
             git -C "$WORK" diff --no-ext-diff --name-only HEAD;
             git -C "$WORK" ls-files --others --exclude-standard; } | sort -u )
...
Skip this step when nothing is staged and no squash is needed. That covers a decline in
Step 3, and a branch that already had one commit, or none, before this run. The branch's
commits go out as they are.
...
When `git config user.signingkey` is set, every commit here must show `G`. `U` counts too,
a good signature on a key that just isn't marked trusted. With no signing key configured,
`N` is expected and is not a failure. A commit from before this run that already showed `N`
is named in the report, not treated as a stop.

### 6d. Freshness and tests

`git rebase` refuses to run while a tracked file is modified and unstaged. Check first:

```bash
git -C "$WORK" status --porcelain --untracked-files=no
```

Anything it lists would block the rebase below. This repo does not use `git stash`, so stop
here and name those files. The user commits or discards them before this step goes on.

Otherwise, bring the branch up to date and run what CI runs:

```bash
git -C "$WORK" fetch origin --quiet
git -C "$WORK" rebase -S origin/main    # drop -S when no signing key is configured
...
6d's rebase replays commits without re-signing them unless `-S` was used. Check the
signatures once more, right before this push:

```bash
git -C "$WORK" log --format='%h %G? %s' origin/main..HEAD
```

Apply the same rule as 6c. `G` or `U` counts as signed when a signing key is configured.
With none, `N` is expected, and a pre-existing `N` is named in the report. A commit that was
signed before 6d's rebase and now shows `N` is a stop: report it and push nothing.
