...
| `tests/e2e-*.sh`, `tests/e2e/**`, `tests/framework/**` or `tests/testdata/**` | `README.md` › Working on the plugin; `tests/README.md` › How a case is checked, Test Results (the fixture list); `AGENTS.md` › Testing; `hack/README.md` › Testing the Plugin | none |
| `.claude/skills/**` | `README.md` › Working on the plugin (the skills paragraph); `development.md` › the skills table (arguments, what each skill needs and leaves behind) and the walkthrough | none |
...

{
  ...
  grep -E '^(\+\+\+ b/|--- a/)(go\.mod|\.github/workflows/.*|hack/.*\.sh|tests/.*\.sh|tests/e2e/.*|tests/framework/.*|tests/testdata/.*|\.claude/skills/.*)$' "$P" | sed 's/^/PATH    /'
