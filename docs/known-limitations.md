...
| `pullSecret` on a BuildConfig that names a `serviceAccount` | not linked. crane carries the account itself, except `builder`, `deployer` and `default` | Run the `oc secrets link` command from the warning on the target | The plugin never edits an account you own, so it cannot attach the secret. [W8](support-matrix.md#w8), and [W67](support-matrix.md#w67) for the three accounts the migration drops |
