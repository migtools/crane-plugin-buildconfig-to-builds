...
| `source.binary` without `asFile` (an extracted archive) | failed | Use a git source, or a single-file binary source | Shipwright's local source uploads a directory, not an archive. [Details](support-matrix.md#what-stops-a-buildconfig-from-converting) |
| More than one of `source.git`, `source.binary`, `source.images` | failed | Split into one BuildConfig per source. For git plus an image, rewrite as a multi-stage Dockerfile | A Shipwright Build has one `spec.source`. [Details](support-matrix.md#what-stops-a-buildconfig-from-converting) |
| More than one `source.images` entry | failed | Rewrite as a multi-stage Dockerfile | One OCI artifact source per Build. [Details](support-matrix.md#what-stops-a-buildconfig-from-converting) |
| Webhook triggers: `GitHub`, `GitLab`, `Bitbucket`, `Generic` | dropped, kept in the `buildconfig-to-shipwright/original-triggers` annotation | Remove or repoint the webhook in your Git provider. Use Pipelines-as-Code or Tekton Triggers to create BuildRuns on push | Shipwright serves no webhook URL. The Triggers component that would is not shipped with the operator. [W50](support-matrix.md#w50), [W51](support-matrix.md#w51) |
| `ImageChange` trigger | dropped, kept in the same annotation | Start a BuildRun from your own automation when the image changes | Nothing in Shipwright watches an image for changes. [W52](support-matrix.md#w52) |
| `ConfigChange` trigger | dropped, kept in the same annotation | Create the first BuildRun yourself. When the Build carries a BuildRun template, apply that | Creating a Build starts nothing. Only a BuildRun starts a build. [W54](support-matrix.md#w54) |
...
| `runPolicy: Serial` and `SerialLatestOnly` | dropped. BuildRuns run concurrently | Serialise runs in your pipeline | Shipwright has no build queue. Every BuildRun starts as soon as it is created. [W43](support-matrix.md#w43), [W44](support-matrix.md#w44) |
...
A note on triggers. Shipwright has a separate [Triggers](https://github.com/shipwright-io/triggers)
project that plans GitHub webhooks, for push and pull request events, and image-change triggers.
As of September 2026 neither is implemented, and the Builds for OpenShift operator does not
ship it. GitLab, Bitbucket and Generic webhooks are outside its stated scope. If that changes,
the webhook and `ImageChange` rows above are the two that would move.
