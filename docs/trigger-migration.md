...
- **Bitbucket** is written for Bitbucket Server, which signs its payloads. Bitbucket Cloud
  does not sign, so a Cloud trigger checks only the `X-Event-Key` header. Give it a listener
  of its own: copy the `buildconfig-generic` EventListener and Route under a new name, move
  the trigger there with the `secretRef` param removed and `eventTypes` set to
  `["repo:push"]`, and treat that URL the way the Generic paragraph below says. Not on the
