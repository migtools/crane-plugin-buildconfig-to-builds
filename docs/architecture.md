...
| 2 | Strategy | inline switch, then `processDockerStrategy` or `processSourceStrategy` | `spec.strategy` | strategy name (`buildah` or `source-to-image`, both strategy-catalog names, or the override from `default-build-strategy`), the strategy params, `spec.env`, `spec.volumes`. W81 when the override is not a valid Kubernetes name, and a placeholder for it in the W76 and W78 commands | Custom and JenkinsPipeline: skipped. Unknown type: failed. A `from` image that cannot be resolved: failed |
...
| 4 | Name check, then pull secret | inline, `commandArg`, `warnInvalidName`, `getPullSecret`, `generateServiceAccount` | `metadata.namespace`, the strategy's `pullSecret`, `spec.serviceAccount` | W81 for each of those names the API server would reject, and a placeholder for it in the W8, W72 and W73 commands; a new ServiceAccount carrying the secret, only when the BuildConfig names no service account | serialization error: failed |
...
| `buildconfig/names.go` | DNS-1123 sanitizing and hash suffixing, and `commandArg`, which keeps an invalid name out of a pasteable command | read every changed line |
...
| 22 | A name the API server would reject never reaches a command a warning tells the operator to paste: `commandArg` puts a placeholder in its place and W81 names the value. `%q` is not enough, because a shell still expands `$(…)` inside double quotes | a hand-edited export can carry any string, and the warning text is copied into a shell | `command_names_test.go` (`TestCommandNamesKeepInvalidNamesOutOfCommands`, `TestCommandNamesNewlineCannotForgeAWarning`, `TestCommandNamesBinaryUploadNamesTheBuild`). ADR-0006 |
| 23 | `sourceStrategy.env` stays in `spec.env`, and `build-env` names each entry as `NAME=$(NAME)` in order. Names that cannot sit inside `$(...)` are left out with W82; `fieldRef` and `resourceFieldRef` entries are kept with W83. Git proxy variables never enter `build-env` | the source-to-image strategy passes `build-env`, not `spec.env`, to s2i. The kubelet fills `$(NAME)` from the step's environment, so every kind of entry resolves as it did on OpenShift and no Secret value is copied into the Build | `strategy_env_test.go` (`TestConvertSourceEnvBuildEnv`, `TestConvertStrategyEnvNoWarning`). ADR-0017 |
