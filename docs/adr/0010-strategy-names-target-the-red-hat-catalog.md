...
- On upstream Shipwright a Docker Build fails to register: there is no `buildah`. A Source
  Build registers only when the BuildConfig was plain enough to write `builder-image` and
  nothing else; any other parameter fails it. Either way upstream stays unsupported, and
  the docs now say why. The plugin still emits no warning about it: it cannot see the cluster (ADR-0001), and a
