...
Reviewed 2026-09-02. Amended 2026-09-10 by ADR-0012 (BUILD-2402): the template also exists for
a ServiceAccount.
...
When `spec.resources` has requests or limits, and since ADR-0012 whenever there is a
...
- One place writes the annotation, behind one check. It was "no requests and no limits
  means no template"; since ADR-0012 it is "no resources and no account means no template".
...
- Anything that wants to ride on the template has to decide first when the template exists.
  ...
  no Build field, widened the gate instead (ADR-0012).
