...
- A name that cannot sit inside `$(...)` (the build-arg rules, plus `(` and `)`) stays in
  `spec.env`, is left out of `build-env`, and is named in W82.
- A `fieldRef` or `resourceFieldRef` entry is passed like any other, and W83 says it now reads
...
- An optional Secret or ConfigMap key the plugin cannot see on the target has the same
  failure mode: if it is missing at BuildRun time, Kubernetes leaves the variable unset,
  and s2i gets the literal text `$(NAME)` instead of the omitted variable OpenShift left.
  The entry stays in `build-env` because the plugin cannot check the target offline; W84
  names it.
