...
Warnings are quoted in the [Warning reference](#warning-reference) at the end, keyed [W1](#w1) to [W77](#w77). Each number is an anchor: `#w12` jumps to [W12](#w12).
...
| a `Secret` or `ConfigMap` volume | Converted, with a warning. The Build will not register (`Registered=False`, reason `UndefinedVolume`) until the strategy declares the volume | `spec.volumes[]`, under the same name | Copy the ClusterBuildStrategy, add an overridable volume with that name and a mount at the original path, and point the Build at the copy. See `docs/volume-migration.md` | The shipped strategies declare no volume under a name of yours. The one name they do declare is `trusted-ca`, which [Build settings](#build-settings) covers. [W25](#w25) per volume, [W26](#w26) once |
...
| `ConfigMap` | `spec.mountTrustedCA: true` and no strategy volume named `trusted-ca`. Named the BuildConfig's name plus `-trusted-ca`, sanitized |
