...
| `spec.mountTrustedCA: true` | Converted, with a warning. A `trusted-ca` volume is added, backed by a new ConfigMap `<build>-trusted-ca` labelled `config.openshift.io/inject-trusted-cabundle: "true"`; only its `ca-bundle.crt` key is mounted | `spec.volumes[]`, as `trusted-ca` | On a cluster without the Cluster Network Operator, fill `ca-bundle.crt` in the ConfigMap yourself. With a strategy other than `buildah` or `source-to-image`, declare an overridable `trusted-ca` volume in it | The Cluster Network Operator fills the ConfigMap, as it does for OpenShift builds. [W76](#w76), and [W77](#w77) with a custom strategy |
...
| `ConfigMap` | `spec.mountTrustedCA: true` and no strategy volume named `trusted-ca`. Named `<build>-trusted-ca` |
...
| <a id="w76" name="w76"></a>W76 | `mountTrustedCA for BuildConfig … relies on the OpenShift Cluster Network Operator injecting the cluster CA bundle into ConfigMap … (label …); on clusters without that injector the … key stays absent and BuildRun pods will fail to mount the … volume until the key is populated manually` |
