...
Warnings are quoted in the [Warning reference](#warning-reference) at the end, keyed [W1](#w1) to [W85](#w85). Each number is an anchor: `#w12` jumps to [W12](#w12).
...
| `metadata.name`, `metadata.namespace` | Converted | `metadata.name`, `metadata.namespace` | Nothing | none, or [W81](#w81) when the namespace is not a valid Kubernetes name |
...
| `dockerStrategy.pullSecret` or `sourceStrategy.pullSecret`, with `spec.serviceAccount` also set | Dropped. The plugin does not touch a named ServiceAccount | | Link the secret to that ServiceAccount on the target: `oc -n <ns> secrets link <sa> <secret> --for=pull,mount` | [W8](#w8), and [W81](#w81) when a name in the command is not a valid Kubernetes name |
| `dockerStrategy.pullSecret` or `sourceStrategy.pullSecret`, no `spec.serviceAccount` | Converted. A new ServiceAccount carrying the secret is generated | a `ServiceAccount` named after the BuildConfig, listed in both `imagePullSecrets` and `secrets`. The BuildRun template names it | Migrate the secret itself; the plugin only references it. On OpenShift, grant the generated account the SCC the strategy needs | none |
| `spec.serviceAccount` | Converted, with a warning. The name is carried into the BuildRun template, which is written for every named account. crane migrates the account itself, its user-created secrets, its RoleBindings and the ClusterRoleBindings, ClusterRoles and SCCs that name it; this plugin passes them through | `spec.serviceAccount` of the BuildRun template | Check the account was in the export. Re-link any secret that was linked with `--for=mount`: crane-lib clears the account's `secrets` list (BUILD-2343). Read the exported `_cluster` resources and check each name against the target before applying them without `--skip-cluster-scoped`: an SCC or ClusterRole is cluster-global, and an exported SCC carries the source cluster's own users and groups, so one that shares a name with an SCC on the target replaces it. Confirm the account holds the SCC the strategy needs | crane carries the account; only its secrets list is dropped. [W9](#w9), or [W72](#w72) for `builder`, `deployer` and `default`. `default` goes whenever crane-lib's KubernetesPlugin runs; `builder` and `deployer` go only when crane-plugin-openshift is one of the stages `crane transform` runs, which it is not in the two-stage command this plugin documents. [W81](#w81) when the name is not a valid Kubernetes name. |
...
| `dockerStrategy.volumes[]` | Converted, with a warning | `spec.volumes[]` | See [Strategy volumes](#strategy-volumes) | Volumes convert, but the strategy has to declare them. [W22](#w22) to [W26](#w26), or [W78](#w78) for a volume named `trusted-ca`, which the shipped strategies already declare |
...
| `dockerStrategy.env[]` with a name Shipwright forbids | Converted and kept, with a second warning naming the entry | `spec.env[]` | Take the entry off the Build and get the value another way, or have a cluster administrator allow the name | Shipwright v0.21.0 leaves a Build whose `spec.env` carries a blocklisted name unregistered. The default list is `LD_*`, `BASH_FUNC_*`, `BASH_ENV`, `ENV`, `CDPATH`, `PYTHONSTARTUP`, `PERL5OPT`, `PERLLIB`, `PERL5LIB`, `RUBYOPT` and `NODE_OPTIONS`. [W79](#w79) |
...
| `sourceStrategy.volumes[]` | Converted, with a warning | `spec.volumes[]` | See [Strategy volumes](#strategy-volumes) | Volumes convert, but the strategy has to declare them. [W22](#w22) to [W26](#w26), or [W78](#w78) for a volume named `trusted-ca`, which the shipped strategies already declare |
| `sourceStrategy.env[]` | Converted | `spec.env[]`, and `spec.paramValues[build-env]` as one `NAME=$(NAME)` item per entry | Nothing, unless a name cannot be passed that way (W82), an entry reads a field (W83), or an entry reads an optional Secret or ConfigMap key (W84) | The kubelet fills each `$(NAME)` from `spec.env` when the build starts, so ConfigMap, Secret and `$(OTHER)` entries reach s2i as they did on OpenShift. s2i writes them into the image as `ENV`, Secret values included, as OpenShift did. A key that is optional and missing at BuildRun time is the exception: Kubernetes leaves the variable unset, and s2i gets the literal text `$(NAME)` instead. [W82](#w82), [W83](#w83), [W84](#w84) |
| `sourceStrategy.env[]` with a name Shipwright forbids | Converted and kept, with a warning naming the entry | `spec.env[]`, and its `NAME=$(NAME)` item in `spec.paramValues[build-env]` | Take the entry off the Build, from `spec.env` and from `build-env` both, and get the value another way, or have a cluster administrator allow the name. Removing it from `spec.env` alone leaves s2i the literal text `$(NAME)` | Shipwright v0.21.0 leaves a Build whose `spec.env` carries a blocklisted name unregistered. The same list as for `dockerStrategy.env[]` above. [W79](#w79) |
...
The four S2I params above, `build-env`, `scripts-url`, `incremental` and `pull-policy`, exist only on a
...
| a `Secret` or `ConfigMap` volume | Converted, with a warning. The Build will not register (`Registered=False`, reason `UndefinedVolume`) until the strategy declares the volume | `spec.volumes[]`, under the same name | Copy the ClusterBuildStrategy, add an overridable volume with that name and a mount at the original path, and point the Build at the copy. See `docs/volume-migration.md` | The shipped strategies declare no volume under a name of yours, with one exception: `trusted-ca`, which they already declare from strategy-catalog commit `cb2432c` onward — see [Build settings](#build-settings). A volume by that name gets the catalog-version check instead, and is mounted at `/var/shipwright/trusted-ca` whatever its original path ([W78](#w78)); any other name gets [W25](#w25) per volume, [W26](#w26) once |
...
`spec.mountTrustedCA` also lands in `spec.volumes[]`, but it is a top-level `spec` field
rather than a strategy volume, so its rows are in [Build settings](#build-settings).
...
| `source.binary` without `asFile` (started with `oc start-build --from-dir`, `--from-archive` or `--from-repo`) | Converted, with a warning | `spec.source: {type: Local, local: {name: local-copy, timeout: 10m}}` | Start each build with `shp build upload <build> <directory>`. The directory becomes the build context, but not all of it: see the upload row in [known-limitations.md](known-limitations.md#converted-but-needs-a-step-from-you) | Nothing feeds a Local source on its own. A BuildRun started without the upload waits out the timeout and fails. `shp` skips files that `.gitignore` lists and symlinks pointing outside the directory, which `--from-dir` sent. [W67](#w67), [ADR-0011](adr/0011-binary-upload-differences-warn-only-what-lasts.md) |
| `source.binary.asFile` | Converted, with a warning. The file name is not represented in the Build | `spec.source: {type: Local, local: {name: local-copy, timeout: 10m}}` | Put the file in a directory under the `asFile` name and start each build with `shp build upload <build> <directory>` | A Local source takes a directory, not one file. [W68](#w68) |
...
| `source.git.httpProxy`, `httpsProxy`, `noProxy` | Converted | `spec.env[]` as `HTTP_PROXY` and `http_proxy`, and likewise for the other two, unless `dockerStrategy.env` or `sourceStrategy.env` already set that exact name (W85) | Nothing | A name `dockerStrategy.env` or `sourceStrategy.env` already sets is left to the strategy's value, matching how OpenShift's own Git proxy applied only to the clone. [W85](#w85) |
...
| `output.imageLabels[]` with `=` in the name | Dropped | | Rename the label, or move the part after the `=` into the value | From Shipwright v0.21.0 the Build CRD carries a CEL rule on `spec.output.labels`, so the API server rejects the whole Build over one such key. [W80](#w80) |
...
| Field | What happens | Where it lands | What you do by hand | Warning |
|---|---|---|---|---|
| `spec.resources` (requests or limits) | Converted, with a warning. The Build cannot hold resources, so a BuildRun template is generated instead. The template also exists, without `stepResources`, when the BuildConfig names an account or the plugin generated one | the annotation `buildconfig-to-shipwright/buildrun-template`, holding a BuildRun with `stepResources` for the strategy's steps and the ServiceAccount | Review the template, then apply it to start a build. With `--default-build-strategy` the step names are unknown and `stepResources` is left out | A Build has no resources field. A BuildRun does. [W48](#w48), or [W47](#w47) with a custom strategy; neither fires for a template that carries only an account, because there are no resources to place. With a binary source the template cannot start the Build at all, whatever it holds: `shp build upload` creates its own BuildRun, takes the account with `--sa-name` and takes no step resources, so each build runs with the strategy's defaults. [W71](#w71) replaces W48 and W47 there, with [W73](#w73) when the template names an account and [W74](#w74) when resources were set |
...
| `spec.mountTrustedCA: true` | Converted, with a warning. A `trusted-ca` volume is added, backed by a new ConfigMap named after the BuildConfig plus `-trusted-ca-migrated`, sanitized, and labelled `config.openshift.io/inject-trusted-cabundle: "true"`; only its `ca-bundle.crt` key is mounted | `spec.volumes[]`, as `trusted-ca`, plus the ConfigMap | Check the target's strategy declares the volume: `oc get clusterbuildstrategy buildah -o jsonpath='{.spec.volumes[*].name}'`. `trusted-ca` reached `buildah` and `source-to-image` in strategy-catalog commit `cb2432c`, and an older catalog leaves the Build `Registered=False`, reason `UndefinedVolume`. With a strategy of your own, declare an overridable `trusted-ca` volume in it. On a cluster without the Cluster Network Operator, fill `ca-bundle.crt` in the ConfigMap yourself | The volume exists on the shipped strategies from `cb2432c` onward, and the Cluster Network Operator fills the ConfigMap, as it does for OpenShift builds. [W76](#w76), and [W77](#w77) with a strategy of your own |
| `spec.mountTrustedCA: true` with a strategy volume already named `trusted-ca` | Dropped. The declared volume is kept, even when it was itself dropped as unsupported | | Migrate the CA source by hand if the declared volume was dropped | The BuildConfig's own volume wins. [W75](#w75), plus [W78](#w78) from converting the declared volume itself when its source is supported |
...
| `--default-build-strategy` | `docker=name,s2i=name` | Names a copy of a catalog strategy, not an upstream Shipwright one (ADR-0010). With a custom name the BuildRun template omits `stepResources` ([W47](#w47)). A name that is not a valid Kubernetes name raises [W81](#w81), and the [W76](#w76) and [W78](#w78) commands show `<strategy>` in its place |
...
| `ConfigMap` | `spec.mountTrustedCA: true` and no strategy volume named `trusted-ca`. Named the BuildConfig's name plus `-trusted-ca-migrated`, sanitized |
...
| `buildconfig-to-shipwright/buildrun-template` | Build | `spec.resources` has requests or limits, or the BuildRun has an account to run as: one the BuildConfig named, or one the plugin generated. `resources: {}` on its own writes nothing |
...
| <a id="w63" name="w63"></a>W63 | Retired by BUILD-2475. Was the failure reason for a binary source without asFile. That source now converts, and W67 carries the warning |
...
| <a id="w65" name="w65"></a>W65 | `error converting Build to unstructured: …`, `error converting ServiceAccount to unstructured: …`, `error converting inline-Dockerfile ConfigMap to unstructured: …`, `error converting trusted CA ConfigMap to unstructured: …`, `error marshaling BuildRun spec for BuildConfig …: …`, `error unmarshaling BuildRun spec for BuildConfig …: …` or `error marshaling BuildRun template for BuildConfig …: …` (a failure reason) |
...
| <a id="w70" name="w70"></a>W70 | Retired by BUILD-2500. Said that sourceStrategy.env did not reach s2i. The plugin now passes it through build-env |
| <a id="w71" name="w71"></a>W71 | `The Build for BuildConfig …/… has a Local source, which starts only through 'shp build upload … <directory>'. That command creates its own BuildRun, so the BuildRun template in annotation … cannot start this Build.` Fires whenever a Local-source Build carries a template, and is followed by [W73](#w73), [W74](#w74) or both |
| <a id="w72" name="w72"></a>W72 | `BuildConfig …/… names ServiceAccount …, which the migration may not carry over: crane-lib's KubernetesPlugin drops default under strip-default-rbac, and crane-plugin-openshift drops builder and deployer, but only when it runs, which it does when it is one of the stages named on crane transform or when no stages are named at all. The BuildRun template names … anyway, so check the target before the first BuildRun: oc -n … get serviceaccount …. If the account is there, leave the BuildRun on it, confirm it holds the SCC the build strategy needs, and link to it any pull secret the other warnings on this Build name. If it is not, create an account for this build, link those secrets to that one, grant it the SCC scoped to itself with oc adm policy add-scc-to-user pipelines-scc -z <sa> -n …, and name it on the BuildRun; the namespace pipeline account already holds the grant and is the shorter path, but it is a shared identity every other Tekton workload in the namespace runs as. Either way, do not grant that SCC to the namespace's default account: every pod that names no account runs as it.` |
...
| <a id="w75" name="w75"></a>W75 | `BuildConfig … sets mountTrustedCA but already declares a strategy volume named … — deferring to the explicit volume and skipping the trusted CA mapping (if the explicit volume was itself skipped as unsupported, migrate its CA source manually)` |
| <a id="w76" name="w76"></a>W76 | `mountTrustedCA for BuildConfig … relies on the OpenShift Cluster Network Operator injecting the cluster CA bundle into ConfigMap … (label …); on clusters without that injector the … key stays absent and BuildRun pods will fail to mount the … volume until the key is populated manually. The volume itself reached the shipped buildah and source-to-image ClusterBuildStrategies in strategy-catalog commit cb2432c, so check the target declares it before you apply: oc get clusterbuildstrategy … -o jsonpath='{.spec.volumes[*].name}'. A catalog older than cb2432c declares no such volume and Shipwright refuses to register the Build (Registered=False, reason UndefinedVolume), whatever the strategy is called` |
| <a id="w77" name="w77"></a>W77 | `mountTrustedCA was mapped to the … volume for BuildConfig …, but the target ClusterBuildStrategy … is not a shipped strategy — Shipwright will reject the Build (Registered=False, reason UndefinedVolume) unless the strategy declares a matching overridable volume: volumes: [{name: …, overridable: true, emptyDir: {}}] plus a volumeMount on the build step` |
| <a id="w78" name="w78"></a>W78 | `Volume … was converted. Unlike other strategy volumes, the shipped buildah and source-to-image ClusterBuildStrategies already declare an overridable volume by this name, from strategy-catalog commit cb2432c onward, so on a target at or after that commit the Build registers without a strategy change — check first: oc get clusterbuildstrategy … -o jsonpath='{.spec.volumes[*].name}'. Those strategies mount it at … and import only its .crt and .pem files into the trust store; if the build expects the files at another path (…), adapt the build or point it at a strategy copy that mounts the volume there. A catalog older than cb2432c, or a strategy of your own, declares no such volume and Shipwright will reject the Build (Registered=False, reason: UndefinedVolume) until you add one: volumes: [{name: …, overridable: true, emptyDir: {}}] plus a volumeMount for '…' on the strategy build step.` |
| <a id="w79" name="w79"></a>W79 | `BuildConfig …/… sets … …, a name Shipwright forbids for security reasons. From Shipwright v0.21.0 a Build carrying it stays unregistered with the reason SpecEnvNameForbidden, so no BuildRun of it ever starts. The entry is kept in spec.env so you can see what to act on: take it off the Build and get the value in another way, or have a cluster administrator allow the name through the build controller's FORBIDDEN_ENV_VAR_NAMES setting.` |
| <a id="w80" name="w80"></a>W80 | `Skipping output imageLabel …: a Shipwright Build rejects a label key containing '='` |
| <a id="w81" name="w81"></a>W81 | `BuildConfig …/… names … …, which is not a valid Kubernetes name, so the target cluster will reject an object with that name. Any command in this Build's warnings shows … in its place: correct the name in the export and convert again.` |
| <a id="w82" name="w82"></a>W82 | `BuildConfig …/… sets sourceStrategy.env …, and a name like that cannot be passed to s2i as NAME=$(NAME). The entries stay in spec.env, which s2i does not see. Rename them, or add each one to the Build's build-env parameter as NAME=VALUE. For an entry read from a Secret that puts the value on the Build in plain text, so rename it instead` |
| <a id="w83" name="w83"></a>W83 | `BuildConfig …/… sets sourceStrategy.env … from a fieldRef or resourceFieldRef. OpenShift read these from the Build object. Shipwright reads them from the BuildRun pod, so metadata.name and labels give the pod's values. Check that the build still gets the value it expects` |
| <a id="w84" name="w84"></a>W84 | `BuildConfig …/… sets sourceStrategy.env … from an optional Secret or ConfigMap key. The entry stays in build-env because the plugin cannot check the target offline. If the key is missing there, Kubernetes leaves the variable unset, the kubelet leaves the literal text $(NAME) in the build-env value, and s2i puts that literal text into the image, where OpenShift left the variable unset. Make sure the key exists, or drop the entry` |
| <a id="w85" name="w85"></a>W85 | `BuildConfig …/… sets … in its strategy env and in source.git proxyConfig. The strategy env value was kept and the Git proxy value was not added. On OpenShift the Git proxy applied only to the clone, and the build saw the strategy env value` |
